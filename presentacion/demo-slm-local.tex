...
\begin{frame}{Los tres tamaños de hoy}
  \centering
  \salida[\scriptsize]{02-modelos}{Los modelos de la demo: ventana, velocidad
  y tiempo total de las cinco tareas}
...
\begin{frame}{Las cinco tareas}
  ...
  \begin{tabular}{@{}clll@{}}
    ...
    3 & Reescritura para el paciente & Reformular sin perder & 8B$^\dagger$ \\
    \midrule
  ...
  \vspace{.8em}
  {\footnotesize $^\dagger$ Con reservas: ninguno cumple los tres umbrales de la
  tarea 3 a la vez. El 8B es el único que \textbf{no enmienda el tratamiento},
  pero se deja un mensaje crítico; el 27B los conserva los trece y lo enmienda.\par}
...
\begin{frame}{Lo que hay que mirar no son los nombres}
  ...
  \vspace{.8em}
  \begin{center}\small
  Los cuatro modelos los conservaron: \bien{0 de 13 destruidos}. El riesgo
  existe y aquí no se materializó.
  \end{center}
  \pause
  \vspace{.6em}
  \begin{center}
...
\begin{frame}{El gradiente}
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    \midrule
    Categoría (3 caracteres) & 6/14 & 9/14 & 10/14 & \bien{13/14} \\
    Código completo & \mal{1/16} & \mal{2/16} & \mal{5/16} & \bien{15/16} \\
    Diagnóstico principal & \mal{no} & \mal{no} & \mal{no} & \mal{no} \\
    \midrule
    Códigos fuera del patrón & 9 & 8 & 8 & \bien{0} \\
    \quad no facturables & 3 & 1 & 3 & \bien{0} \\
    \quad \textbf{que no existen} & \mal{1} & \mal{1} & \mal{1} & \bien{0} \\
    \bottomrule
  \end{tabular}

  \vspace{.8em}
  {\footnotesize Los 22 códigos fuera del patrón, comprobados uno a uno.
  \emph{No facturable} = existe como categoría pero le falta el último carácter:
  en el CMBD es igual de inservible.\par}

  \vspace{.6em}
  \alert{Sabe de qué va. No sabe el código.}\\[.2em]
  {\small \textbf{Ninguno acierta el principal}, tampoco el modelo de frontera:
  la norma de combinación I13.0 exige conocer la regla de secuenciación, no leer
  el texto. Eso no se arregla con parámetros.}
\end{frame}
...
\begin{frame}{El esquema le obliga a devolver algo con forma de código}
  ...
  \begin{columns}[T]
    \column{.45\textwidth}\centering
      {\setlength{\altocaptura}{0.42\textheight}%
      \captura{11-ecie-sin-resultados.png}{eCIE-Maps buscando \texttt{J41.9}:
      «sin resultados»}}

      \vspace{.6em}
      {\small Cada modelo se inventó \textbf{uno}, y ninguno coincide:\\[.3em]
      \mal{\texttt{J41.9}} el 3B\\
      \mal{\texttt{K50.2}} el 8B\\
      \mal{\texttt{H36.0}} el 27B\par}
    \column{.53\textwidth}
      \salida[\tiny]{11-codigos}{Los 22 códigos fuera del patrón, con su
      veredicto}
  \end{columns}

  \vspace{.3em}
  \centering\small \texttt{J41.9} tiene el mismo aspecto que \texttt{J41.0},
  que sí existe. \mal{El esquema no distingue uno del otro.}
\end{frame}
...
\begin{frame}{Resumen de la matriz}
  ...
  \vspace{.8em}
  {\footnotesize La columna \textbf{Opus~5} es el techo de referencia: los
  mismos prompts en un modelo de frontera, ejecutados a ciegas. Aparece cuando
  se recoge con \texttt{make referencia-recoger}.\par}
\end{frame}
...
\begin{frame}{El techo}
  ...
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    \textbf{Tarea} & \textbf{3B} & \textbf{8B} & \textbf{27B} & \textbf{Opus~5} \\
    \midrule
    1 Anonimización & \mal{30/46} & \mal{33/46} & \bien{46/46} & \bien{46/46} \\
    2 Extracción & 13/14 & \bien{14/14} & \bien{14/14} & \bien{14/14} \\
    3 Reescritura & \mal{11/13} & 12/13 & \bien{13/13} & \bien{13/13} \\
    \midrule
    4 Codificación & \mal{1/16} & \mal{2/16} & \mal{5/16} & \bien{15/16} \\
    5 Razonamiento & \mal{1/3} & 2/3 & \bien{3/3} & \bien{3/3} \\
    \bottomrule
  \end{tabular}

  \vspace{.8em}
  \begin{center}\footnotesize
  La última columna se obtiene enviando el informe a un modelo de frontera, con
  los mismos prompts y \textbf{a ciegas}: sin los patrones de evaluación
  delante.\\
  \textbf{Sólo es posible porque el informe es sintético.}
  \end{center}
  \vspace{.4em}
  \centering\small Y es el techo, no una opción: para un informe real hay que
  enviar el documento.
\end{frame}
...
\begin{frame}{Medido, no estimado}
  \begin{columns}[c]
    \column{.56\textwidth}
      \salida[\tiny]{13-velocidad}{tok/s medidos, tarea a tarea}
    \column{.42\textwidth}\centering
      {\setlength{\altocaptura}{0.38\textheight}%
      \captura{14-monitor-memoria.png}{Monitor de actividad con la memoria
      ocupada al cargar el 27B}}
  \end{columns}
  \vspace{.4em}
  \centering\footnotesize Cifras de esta máquina, no de una tabla. El 27B tarda
  \textbf{20 minutos por tarea}; el 3B, \textbf{16 segundos}.
\end{frame}
